...
def get_data(
    ...
    or_filters = frappe._dict(or_filters or {})
    ...
    if view_type == "kanban":
        ...
        for kc in kanban_columns:
            ...
            if (
                ...
                if order:
                    ...
                    column_data = frappe.get_list(
                        doctype,
                        fields=rows,
                        filters=convert_filter_to_tuple(
                            doctype, column_filters),
                        order_by=order_by,
                        page_length=page_length,
                    )
                ...
                all_count = len(
                    frappe.get_list(
                        doctype, filters=convert_filter_to_tuple(
                            doctype, new_filters)
                    )
                )
            ...
            data.append(
                {"column": kc, "fields": kanban_fields, "data": column_data})
    ...
    if doctype == "Contact":
        for d in data:
            unread = frappe.db.count(
                "WhatsApp Message",
                filters={"is_read": 0, "reference_name": d.name},
            )
            if unread:
                d["unread"] = unread
            last_message = frappe.get_list(
                "WhatsApp Message",
                fields=["message"],
                filters={"reference_name": d.name, "content_type": "text"},
                limit_page_length=1,
                as_list=False,
                order_by="creation desc"
            )
            if last_message:
                d["last_message"] = last_message[0].message if last_message else None
...
def get_next(
    ...
    if not filters:
        ...
        finalFilters.extend(filters)
    elif isinstance(filters, str):
        try:
            parsed_filters = json.loads(filters)
            if isinstance(parsed_filters, dict):
                finalFilters.append(parsed_filters)
            elif isinstance(parsed_filters, list):
                finalFilters.extend(parsed_filters)
        except json.JSONDecodeError:
            finalFilters.append({filters})
    if not or_filters:
        finalOrFilters = []
    elif isinstance(or_filters, dict):
        finalOrFilters.append(or_filters)
    elif isinstance(or_filters, list):
        finalOrFilters.extend(or_filters)
    elif isinstance(or_filters, str):
        try:
            parsed_filters = json.loads(or_filters)
            if isinstance(parsed_filters, dict):
                finalOrFilters.append(parsed_filters)
            elif isinstance(parsed_filters, list):
                finalOrFilters.extend(parsed_filters)
        except json.JSONDecodeError:
            finalOrFilters.append({or_filters})
    ...
    if sort_field == "creation" or sort_field == "lead_name" or sort_field == "name" or sort_field == "idx" or sort_field == "status" or sort_field == "converted":
        finalFilters.append([doctype, sort_field, condition,
                            frappe.get_value(doctype, value, sort_field)])
    else:
        finalFilters.append([doctype, "modified", condition,
                            frappe.get_value(doctype, value, "modified")])
