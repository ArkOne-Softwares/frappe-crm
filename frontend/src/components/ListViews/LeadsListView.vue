<template>
  <div class="flex flex-row justify-stretch">
    <div>
      <ListView class="w-3/4" :class="$attrs.class" :columns="columns" :rows="rows" :options="{
            ...
            <NestedPopover>
              <template #target>
                <div class="flex items-center">
                  <Button :label="column.label" variant="ghosted">
                  </Button>
                </div>
              </template>
              <template #body="{ close }">
                <div class="my-2 rounded-lg border border-gray-100 bg-white shadow-xl">
                  <div class="min-w-72 p-2 sm:min-w-[400px]">
                    <TextInput :type="'search'" size="sm" variant="subtle" placeholder="Placeholder" :disabled="false"
                      v-model="searchValue[column.key]" modelValue="">
                      <template #suffix>
                        <Button variant="ghosted" @click="(event) => {
        ...
        <ListRows :rows="rows" v-slot="{ idx, column, item, row }">
          <div v-if="column.key === '_assign'" class="flex items-center">
            <MultipleAvatar :avatars="item" size="sm" @click="(event) =>
              emit('applyFilter', {
                event,
                idx,
                column,
          ...
          <ListRowItem v-else :item="item">
            <template #prefix>
              ...
              <div v-else-if="column.key === 'lead_name'">
                <Avatar v-if="item.label" class="flex items-center" :image="item.image" :label="item.image_label"
                  size="sm" />
              </div>
              ...
              <div v-else-if="column.key === 'lead_owner'">
                <Avatar v-if="item.full_name" class="flex items-center" :image="item.user_image" :label="item.full_name"
                  size="sm" />
              </div>
            ...
            <template #default="{ label }">
              ...
              <div v-else-if="column.key === '_liked_by'">
                <Button v-if="column.key == '_liked_by'" variant="ghosted"
                  :class="isLiked(item) ? 'fill-red-500' : 'fill-white'" @click.stop.prevent="() =>
                    emit('likeDoc', {
                      name: row.name,
                      liked: isLiked(item),
                    })
              ...
              <div v-else-if="column.key === 'sla_status'" class="truncate text-base">
                <Badge v-if="item.value" :variant="'subtle'" :theme="item.color" size="md" :label="item.value" @click="(event) =>
                  emit('applyFilter', {
                    event,
                    idx,
                    column,
              ...
              <div v-else class="truncate text-base" @click="(event) =>
                emit('applyFilter', {
                  event,
                  idx,
                  column,
      ...
      <ListFooter v-if="pageLengthCount" class="border-t sm:px-5 px-3 py-2" v-model="pageLengthCount" :options="{
        rowCount: options.rowCount,
        totalCount: options.totalCount,
      }" @loadMore="emit('loadMore')" />
      <ListBulkActions ref="listBulkActionsRef" v-model="list" doctype="CRM Lead" />
    </div>
    <div class="w-1/4 bg-green-50">
      {{selectedLead}}
      <Lead v-if="selectedLead" :leadId="selectedLead" :key="selectedLead"/>
      <div v-else>
        <div class="flex items">
          <div class="flex items-center justify-center w-full h-32">
            <div class="text-center">
              <div class="text-2xl font-semibold text-gray-500">Select a lead to view details</div>
            </div>
          </div>
        </div>
      </div>
