<template>
  <div>
    <div v-for="(group, date) in groupedMessages" :key="date">
      ...
      <div v-for="(whatsapp, index) in group" :key="whatsapp.name">
        <div v-if="index === firstUnreadIndex(date)" class="unread-label">Unread</div>
        <div
            ...
            <div class="flex gap-2 justify-between">
              ...
              <div class="-mb-1 flex shrink-0 items-end gap-1 text-gray-600">
                <Tooltip :text="dateFormat(whatsapp.creation, 'ddd, MMM D, YYYY')">
                  <div class="text-2xs">
                    {{
                      new Date(whatsapp.creation).toDateString() ===
...
const firstUnreadIndex = (date) => {
  const group = groupedMessages.value[date];
  if (!group) return -1;
  console.log("group", group);
  return group.findIndex((message) => !message.is_read);
};
